...
float Perceptron::net(const vector<float>* input)
{
  return weightSum(input->begin(),input->end(),weights.begin(),weights.end());
}

float Perceptron::net(vector<float> input)
{
  prepare(&input);
  return net(&input);
}
